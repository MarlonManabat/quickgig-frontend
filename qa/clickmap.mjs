...
import cfg from './clickmap.config.mjs';
import { watchConsoleAndNetwork } from './utils.mjs';

async function collectClickables(page) {
  return page.$$(':is(a,button,[role="button"],[data-testid],.btn,.button,.link)');
}
async function shouldSkip(el, excludes) {
  const txt = (await el.textContent())?.trim() || '';
  for (const x of excludes) {
    if (txt.match(new RegExp(x.replace(/^.*text\("(.+)"\).*$/, '$1'), 'i'))) return true;
  }
  return false;
}

async function scan(base, path, clicks) {
  ...
  for (const el of els.slice(0, cfg.maxClicksPerPage)) {
    try {
      ...
      errs.push({ type: 'click', text: String(e) });
    }
  }
...
(async () => {
  ...
  for (const b of bases) {
    for (const p of cfg.entryPaths) {
      const e = await scan(b, p, clicks);
      allErrs.push(...e);
    }
  }
  await fs.writeFile('artifacts/clickmap-summary.json', JSON.stringify({ clicks, errors: allErrs }, null, 2));
  if (allErrs.length) {
    console.error('CLICKMAP FAIL:', allErrs.slice(0, 10));
    process.exit(1);
  }
})();
