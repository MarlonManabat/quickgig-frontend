  ...
  useEffect(() => {
    ...
    if (tab === "profiles") {
      supabase
        .from("profiles")
        .select("*")
        .or("hidden.eq.true,blocked.eq.true")
        .order("created_at", { ascending: false })
    ...
    <Shell>
      ...
      <div className="flex gap-4 mb-4 text-sm">
        <button
          ...
          className={tab === "reports" ? "font-semibold" : ""}
        >
          Reports
        </button>
        <button
          onClick={() => setTab("gigs")}
          className={tab === "gigs" ? "font-semibold" : ""}
        >
          Gigs
        </button>
        <button
          onClick={() => setTab("profiles")}
          className={tab === "profiles" ? "font-semibold" : ""}
        >
          Profiles
        </button>
      </div>
          ...
          <tbody>
            {reports.map((r) => (
              <tr key={r.id} className="border-b border-slate-800">
                ...
                <td className="py-2">
                  {r.kind === "gig" && (
                    <Link className="underline" href={`/gigs/${r.target_id}`}>
                      {r.target_id}
                    </Link>
                  )}
            ...
            <li key={p.id} className="border border-slate-800 p-2 rounded">
              <div className="flex justify-between text-sm">
                ...
                <div className="space-x-2">
                  {p.hidden && (
                    <button
                      onClick={() => act("unhide", "profile", p.id)}
                      className="underline"
                    >
                      Unhide
                    </button>
                  ...
                  {p.blocked && (
                    <button
                      onClick={() => act("unban", "profile", p.id)}
                      className="underline"
                    >
                      Unban
                    </button>
