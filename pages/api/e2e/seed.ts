...
export default async function handler(
  ...
  const employerEmail =
    process.env.E2E_EMPLOYER_EMAIL || "e2e-employer@quickgig.ph";
  const workerEmail = process.env.E2E_WORKER_EMAIL || "e2e-worker@quickgig.ph";
  const employerPass = process.env.E2E_EMPLOYER_PASSWORD || "E2e!Pass123";
  const workerPass = process.env.E2E_WORKER_PASSWORD || "E2e!Pass123";
  ...
  if (employer_id || worker_id) {
    await supabase
      ...
      .upsert(
        [
          employer_id
            ? { user_id: employer_id, role: "employer", tickets: 3 }
            : undefined,
          worker_id
            ? { user_id: worker_id, role: "worker", tickets: 3 }
            : undefined,
