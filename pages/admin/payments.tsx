...
export default function AdminPayments({ initial=[] as any[] }){
  ...
  const approve = async (proof:any)=>{
    await fetch('/api/admin/payment-proofs/set-status',{method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ id: proof.id, status:'approved' })})
    await supa.rpc('credit_tickets_admin', { p_user: proof.user_id, p_tickets: Number(process.env.NEXT_PUBLIC_TICKETS_PER_PROOF ?? 5), p_reason: 'payment_proof', p_ref: proof.id })
    location.reload()
  }
        ...
        {initial.map((r:any)=>(
          <li key={r.id} className="border rounded p-3">
            ...
            <div className="flex gap-2">
              <button onClick={()=>approve(r)} className="px-3 py-1 rounded bg-green-600 text-white">Approve</button>
