...
async function upsertProfiles() {
  const profiles = [
    { email: 'demo-user@quickgig.test', full_name: 'Demo User', is_admin: false, can_post: false },
    { email: adminEmail, full_name: 'Demo Admin', is_admin: true, can_post: true },
  ];
...
async function upsertGigs(ownerEmail) {
  const { data: owner } = await supabase
    .from('profiles')
    .select('id')
    .eq('email', ownerEmail)
    .single();
  const ownerId = (owner && owner.id) || ownerEmail;
  const gigs = [
