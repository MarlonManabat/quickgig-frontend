      ...
      if (
        ...
        !el
          ...
          .some(
            (a) =>
              a.getKind() === SyntaxKind.JsxAttribute &&
              (a.getNameNode().getText() === "as" ||
                a.getNameNode().getText() === "params"),
          )
      ) {
        // Swap Link -> LinkSafe and add params placeholder
        el.getTagNameNode().replaceWithText("LinkSafe");
        const first = el.getAttributes()[0];
        el.insertAttribute(first ? el.getAttributes().indexOf(first) + 1 : 0, {
          kind: SyntaxKind.JsxAttribute,
          name: "params",
          initializer: "{{ /* TODO: supply params */ }}",
        });
        // Add import if missing
        const imp = sf
          .getImportDeclarations()
          .find((d) => d.getModuleSpecifierValue() === "@/components/LinkSafe");
        if (!imp)
          sf.addImportDeclaration({
            moduleSpecifier: "@/components/LinkSafe",
            defaultImport: "LinkSafe",
          });
        edits++;
        modified = true;
      }
  ...
  sf.getDescendantsOfKind(SyntaxKind.CallExpression).forEach((call) => {
    ...
    if (/\[[^\]/]+?\]/.test(raw)) {
      call.replaceWithText(
        `safePush(router, '${raw}', {/* TODO: supply params */})`,
      );
      // ensure import
      const hasImp = sf
        .getImportDeclarations()
        .some((d) => d.getModuleSpecifierValue() === "@/utils/safeNav");
      if (!hasImp)
        sf.addImportDeclaration({
          moduleSpecifier: "@/utils/safeNav",
          namedImports: ["safePush"],
        });
      edits++;
      modified = true;
    }
