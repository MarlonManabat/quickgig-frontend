...
(async () => {
  ...
  try {
    const { data: existing } =
      await supabase.auth.admin.getUserByEmail(demoEmail);
  ...
  if (!(await first("applications"))) {
    ...
    if (g?.[0])
      await supabase
        .from("applications")
        .insert({ gig_id: g[0].id, applicant: "seed-applicant" });
  }
  // demo thread+message
  ...
  // mark admin
  const adminEmail =
    process.env.SEED_ADMIN_EMAIL || process.env.ADMIN_EMAILS?.split(",")[0];
  if (adminEmail) {
    await supabase
      .from("profiles")
