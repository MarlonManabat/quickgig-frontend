...
const files = await fg(["{app,src,components,pages}/**/*.{ts,tsx,jsx}"], {
  ignore: ["**/node_modules/**"],
});
let violations = 0;
for (const f of files) {
  ...
  if (
    s.match(/<button(?=[^>]*type=["']?submit)/) &&
    s.match(/onClick=.*router\.push/)
  ) {
...
if (violations) {
  console.error(`Found ${violations} potential nav issues`);
  process.exit(1);
}
