...
export default function ApplicantInterviewsPage() {
  ...
  useEffect(() => {
    listByUser('applicant').then(setItems).catch(() => setItems([]));
  }, []);
  const act = async (id: string, status: Interview['status']) => {
    ...
    toast(
      status === 'accepted'
        ? t('interviews.accepted')
        : status === 'declined'
        ? t('interviews.declined')
        : t('interviews.updated'),
    );
  };
  return (
...
export async function getServerSideProps(context: GetServerSidePropsContext) {
  ...
  if (user.isEmployer)
    return { redirect: { destination: '/employer/interviews', permanent: false } };
  return { props: {} };
}
