...
do $$
begin
  create policy if not exists "jobs_admin_all" on public.jobs
...
do $$
begin
  create policy if not exists "tickets_read_own" on public.tickets_ledger
...
do $$
begin
  create policy if not exists "tickets_insert_own" on public.tickets_ledger
...
do $$
begin
  create policy if not exists "tickets_admin_all" on public.tickets_ledger
    for all using (exists(select 1 from public.profiles p where p.id = auth.uid() and p.is_admin));
exception
  when duplicate_object then null;
end $$;
