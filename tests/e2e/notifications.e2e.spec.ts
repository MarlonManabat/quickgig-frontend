...
import { requireEnvOrSkip } from "./_utils/ci";
...
test("notifications flow", async ({ page }) => {
  const key = requireEnvOrSkip('SUPABASE_SERVICE_ROLE_KEY');
  ...
  await expect(page.locator('a[aria-label="Notifications"] span')).toHaveCount(
    0,
  );
