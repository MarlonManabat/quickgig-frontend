...
test.describe('[full-e2e] tickets gate hire', () => {
  test('hire blocked without tickets, succeeds after credit', async ({ page }) => {
    ...
    // Seed scenario
    const seed = await ctx.post('/api/qa/seed-hire-scenario', { data: { employerEmail: EMPLOYER, workerEmail: WORKER } })
    ...
    const blocked = await Promise.race([
      page.waitForURL(/\/pay/i, { timeout: 6000 }).then(() => true).catch(() => false),
      page.getByText(/out of tickets|add tickets/i).waitFor({ timeout: 6000 }).then(() => true).catch(() => false),
    ])
    ...
    const success = await Promise.race([
      page.getByText(/hired|accepted|confirmed/i).waitFor({ timeout: 8000 }).then(() => true).catch(() => false),
      page.waitForURL(new RegExp(`/applications/${data.applicationId}(?!$)`), { timeout: 8000 }).then(() => true).catch(() => false),
    ])
